...
export function CreatureEditor({
  ...
  const { state, dispatch, addParticle, addConstraint, addMuscle } =
    useEditorState(initialTopology);
    ...
    (id: string) => {
      if (state.tool === "select") {
        ...
        if (!state.pendingConnection) {
          ...
          const p1 = state.topology.particles.find(
            (p) => p.id === state.pendingConnection,
          );
          const p2 = state.topology.particles.find((p) => p.id === id);
          if (p1 && p2) {
            const dist = Math.hypot(
              p2.initialPos.x - p1.initialPos.x,
              p2.initialPos.y - p1.initialPos.y,
            );
  ...
  const handleConstraintClick = useCallback(
    (id: string) => {
      if (state.tool === "select")
        dispatch({ type: "SELECT", element: { type: "constraint", id } });
      else if (state.tool === "delete")
        dispatch({ type: "DELETE_ELEMENT", elementType: "constraint", id });
    },
    [state.tool, dispatch],
  );
  ...
  const handleMuscleClick = useCallback(
    (id: string) => {
      if (state.tool === "select")
        dispatch({ type: "SELECT", element: { type: "muscle", id } });
      else if (state.tool === "delete")
        dispatch({ type: "DELETE_ELEMENT", elementType: "muscle", id });
    },
    [state.tool, dispatch],
  );

  const handleParticleDrag = useCallback(
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden rounded-xl border border-border ml-4">
      <div className="flex items-center justify-between h-14 px-4 border-b border-border bg-card rounded-l-xl shrink-0">
