...
export function TrainingSessionsTable({
  ...
  return (
    <section id="training-sessions" className="mt-10 scroll-mt-8">
      ...
      <div className="overflow-hidden rounded-xl corner-superellipse/1.33 border border-border bg-card shadow-sm">
        {sessions.length === 0 ? (
          <Empty className="min-h-52">
            <EmptyHeader>
              ...
              <EmptyDescription>
                Train a creature and save its progress to create a resumable
                checkpoint.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
                ...
                {sessions.map((session) => (
                  <TableRow key={session.id} className="group">
                    ...
                    <TableCell className="text-muted-foreground">
                      {session.creatureName}
                    </TableCell>
                    <TableCell className="text-right font-mono tabular-nums">
                      {session.bestDistance === null
                        ? "-"
                        : numberFormat.format(session.bestDistance)}
                    </TableCell>
                    ...
                    <TableCell className="pr-5 text-right">
                      <Button asChild variant="outline" size="sm">
                        <Link
                          href={`/dashboard/creatures/${session.creatureId}/train?session=${session.id}`}
                        >
