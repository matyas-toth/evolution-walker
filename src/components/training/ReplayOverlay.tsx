...
import type { LocomotionMetrics, PackedTrainingReplay, Topology } from "@/core/types";
...
function ReplayCanvas({ topology, replay, frameIndex }: ReplayCanvasProps) {
  ...
  const particleIndices = useMemo(
    () =>
      new Map(
        topology.particles.map((particle, index) => [particle.id, index]),
      ),
    [topology],
  );
  ...
  const fixedCamera = useMemo(() => {
    ...
    const horizontalScale =
      (size.width - 28) / (horizontalSpan + horizontalPadding * 2);
    const distanceAboveGround = Math.max(1, replay.groundY - courseBounds.minY);
    const verticalScaleAbove =
      (groundVisualY - 22) / (distanceAboveGround * 1.1);
    const distanceBelowGround = Math.max(0, courseBounds.maxY - replay.groundY);
    const verticalScaleBelow =
      distanceBelowGround > 0
    ...
    return {
      ...
      scale: Math.max(
        0.01,
        Math.min(1.25, horizontalScale, verticalScaleAbove, verticalScaleBelow),
      ),
    };
  ...
  useEffect(() => {
    ...
    const zone = replay.targetZone;
    const { cameraCenterX, groundVisualY, horizontalPadding, scale } =
      fixedCamera;
    ...
    const drawConnection = (p1Id: string, p2Id: string) => {
      ...
      context.beginPath();
      context.moveTo(
        replay.positions[p1Offset],
        replay.positions[p1Offset + 1],
      );
      context.lineTo(
        replay.positions[p2Offset],
        replay.positions[p2Offset + 1],
      );
      context.stroke();
    };
    ...
    for (const constraint of topology.constraints)
      drawConnection(constraint.p1Id, constraint.p2Id);

    context.lineWidth = 3 / scale;
    context.strokeStyle = "oklch(0.72 0.17 162)";
    for (const muscle of topology.muscles)
      drawConnection(muscle.p1Id, muscle.p2Id);
  ...
  return (
    <div ref={containerRef} className="w-full h-full">
      <canvas
        ...
        aria-label="Exact winning trajectory replay"
      />
    </div>
  );
}
...
interface ReplayOverlayProps {
  ...
  isSaving: boolean;
  metrics: LocomotionMetrics | null;
    ...
    <div className="absolute inset-0 z-50 flex flex-col items-center justify-center animate-in fade-in duration-700 bg-background/95">
      <Card className="w-full max-w-xl shadow-xl border-border/50 bg-card/50">
        ...
        <CardContent className="space-y-6">
          <div
            className="relative w-full rounded-lg overflow-hidden border border-border/50 bg-background/50 shadow-inner"
            style={{ height: 260 }}
          >
          ...
          {metrics ? (
            <div className="grid grid-cols-[auto_1fr] gap-x-5 gap-y-2 rounded-lg border border-border/50 bg-muted/30 p-3">
              <div className="row-span-2 flex min-w-20 flex-col items-center justify-center border-r border-border/60 pr-4">
                <span className="font-mono text-3xl font-semibold text-sky-500">{metrics.locomotionQuality.toFixed(0)}</span>
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Gait grade</span>
              </div>
              <div className="text-sm font-medium capitalize">{describeGait(metrics)}</div>
              <div className="grid grid-cols-3 gap-2 text-[10px] text-muted-foreground">
                <span>Contact <b className="text-foreground">{metrics.contactUtilization.toFixed(0)}</b></span>
                <span>Traction <b className="text-foreground">{metrics.traction.toFixed(0)}</b></span>
                <span>Carriage <b className="text-foreground">{metrics.carriage.toFixed(0)}</b></span>
                <span>Smooth <b className="text-foreground">{metrics.smoothness.toFixed(0)}</b></span>
                <span>Periodic <b className="text-foreground">{metrics.periodicity.toFixed(0)}</b></span>
                <span>Efficient <b className="text-foreground">{metrics.energyEfficiency.toFixed(0)}</b></span>
              </div>
            </div>
          ) : null}
