...
type TrainingSessionSummary = Awaited<
  ReturnType<typeof getTrainingSessions>
>[number];
  ...
  const sortedSessions = [...sessions].sort((a, b) => {
    switch (sortBy) {
      ...
      case "efficiency":
        ...
        const effA =
          (a.bestFitness / Math.max(1, a.generation)) *
          (a.targetDistance / 1000);
        const effB =
          (b.bestFitness / Math.max(1, b.generation)) *
          (b.targetDistance / 1000);
        return effB - effA;
      default:
        return 0;
    }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-[400px] sm:w-[540px] flex flex-col p-0 bg-card border-border">
        ...
        <div className="flex-1 overflow-y-auto p-4 py-2 space-y-4">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-muted-foreground border border-dashed rounded-xl ">
              <History className="w-8 h-8 mb-2 opacity-20" />
              <p className="text-sm">No saved runs yet.</p>
              <p className="text-xs mt-1 max-w-[200px] text-center">
                Start a training simulation and save your progress.
              </p>
            </div>
          ) : (
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 bg-card rounded-lg p-2 border border-border/50 text-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm font-mono font-bold text-primary">
                      {Math.round(session.bestFitness)}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 mt-auto">
                  <Button
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/creatures/${creatureId}/train?session=${session.id}`,
                      )
                    }
                  >
