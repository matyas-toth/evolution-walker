...
export function CreaturesGrid({ creatures: initial }: CreaturesGridProps) {
  ...
  const [runsBrowserCreature, setRunsBrowserCreature] =
    useState<CreatureData | null>(null);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/creatures/${deleteTarget.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-normal">Creatures</h1>
      ...
      {creatures.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border bg-card!  border-border py-16">
            ...
            >
              <div className="h-40 bg-background rounded-xl border-b border-border p-4 relative">
                ...
                {creature.hasReachedTarget && (
                  <div
                    className="absolute top-3 right-3 bg-amber-500/20 text-amber-500 p-1.5 rounded-md  backdrop-blur-sm shadow-sm"
                    title="Has a run that reached the target"
                  >
              ...
              <div className="flex items-center justify-between p-4  bg-card">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {creature._count?.trainingSessions || 0} run
                    {(creature._count?.trainingSessions || 0) !== 1 ? "s" : ""}
                  </p>
                </div>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      router.push(`/dashboard/creatures/${creature.id}/train`)
                    }
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This creature and its topology will
